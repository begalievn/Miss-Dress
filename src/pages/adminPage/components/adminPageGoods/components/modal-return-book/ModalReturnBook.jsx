...
const ModalReturnBook = ({open, handleClose}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await axios.post('http://67.207.94.104:9080/api/v1/admin/return-book', formData,
      {
        headers: {
    ...
    >
      <Box sx={style}>
        <div className={classes.container}>
          <h2 className={classes.title}>Вернуть книгу</h2>
          <form onSubmit={handleSubmit}>
            ...
            <button className={classes.submit} type="submit" onClick={handleSubmit}>Вернуть книгу</button>
          </form>
        </div>
      </Box>
    </Modal>
