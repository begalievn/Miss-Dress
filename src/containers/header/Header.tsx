...
const Header = () => {
  ...
  const burgerMenuModal = useAppSelector((state) => state.ModalSlice.burgerMenu);
  const menuProfile = useAppSelector((state) => state.ModalSlice.menuProfile);
  const userState = useAppSelector((state) => state.AuthorizationUserSlice.userState);
  ...
  return (
    <>
      <div className={style.mainBlock} >
        <div className={style.container}>
          <div className={style.burgerMenu_logo} >
            <img src={burgerMenu} onClick={() => animationfunc(burgerMenuModal)} className={style.burgerMenu} alt="" />
            <Link to="/">
              <Logo />
            </Link>
          </div>
              ...
              <Link to={page.link} key={page.id}>
                <li className={
                  location.pathname === page.link
                    ? style.activeNavbar
                    : ""
                } onClick={() => modalFunc("close")}>{page.name}</li>
              </Link>
            ))}
          </ul>
          <div className={style.icons} >
            {iconsArr.map((icon, index) => (
              ...
              <div className={style.shopping_img_count}>
                <img src={icon.url} onClick={() => modalFunc(icon.name)} alt="" className={style[icon.class]} />
      ...
      {burgerMenuModal ? <HeaderBurgerMenu animation={animation} setAnimation={setAnimation} /> : null}
